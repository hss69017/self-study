...
# 게임 종료 메시지 / Time Over(시간 초과 실패), Mission Complete(성공), Game Over(캐릭터 공에 맞음)
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx # 해당 무기 없이개 위한 값 설정
                ball_to_remove = ball_idx # 해당 공 없애기 위한 값 설정

                # 가장 작은 크기의 공이 아니라면 작은 크기의 공으로 나누기
                if ball_img_idx < 3:
                    # 현재 공 크기 정보를 가지고 옴
                    ...
                    balls.append({
                        ...
                        "img_idx": ball_img_idx + 1, # 어떤 크기의 공을 쓸지?
                        "to_x": -3, # 공의 x축 이동뱡향
                        "to_y": -6, # y축 이동방향
