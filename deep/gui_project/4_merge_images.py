...
def add_file():
    files = filedialog.askopenfilenames(title = "Choose your image file(s).", filetypes = (("PNG file", "*.png"), ("all file", "*.*")), \
        initialdir = "/Users/gunju/Desktop/self study/python") # initial file path
...
def browse_dest_path():
    folder_selected = filedialog.askdirectory(initialdir = "/Users/gunju/Desktop/self study/python/deep/gui_project")
...
# merge image
def merge_image():
    # print(list_file.get(0, END)) # take every file list
    ...
    # the biggest width, sum of height
    max_width, total_height = max(widths), sum(heights)
    
    # sketchbook
    result_img = Image.new("RGB", (max_width, total_height), (255, 255, 255))
    y_offset = 0 # 다음 이미지 붙이는 y 좌표를 이전 이미지 붙인 곳부터 시작하게 초기화
    # for img in images:
    ...
    for idx, img in enumerate(images):
        ...
        progress = (idx + 1) / len(images) * 100 # progress percentage
        p_var.set(progress)
        progress_bar.update()

    dest_path = os.path.join(txt_dest_path.get(), "GJ_photo.jpg")
    result_img.save(dest_path)
    msgbox.showinfo("notice", "The work is done.")
